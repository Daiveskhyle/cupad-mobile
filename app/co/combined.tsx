...
type Edit={savings:string;installment:string;installmentCount:string;withdrawalType:string;withdrawal:string;notes:string};
type Settings={
...
const [refreshing,setRefreshing]=useState(false);
 ...
 const installment=(r:Row)=>Number(r.loan?.inst_amt||0)||((Number(r.loan?.total_payable||0))/(Number(r.loan?.num_installments)||23));
 const principal=(r:Row)=>Number(r.loan?.principal||r.loan?.loan_amount||r.loan?.disbursed_amount||r.loan?.amount||r.loan?.total_payable||0);
 ...
 const allowedMax=(r:Row)=>Math.max(0,Math.min(Number(settings.max_installments_per_payment)||3,remainingInstallments(r)||Number(settings.max_installments_per_payment)||3));
 ...
 const selectAdjustment=(r:Row,type:'withdrawal'|'return')=>{const blocked=blockedTypes(settings);if(blocked.includes(type)||blocked.includes(type==='withdrawal'?'deduct':'return')){Alert.alert('Adjustment disabled','This adjustment type is disabled by the administrator.');return}const pct=type==='withdrawal'?Number(settings.buffer_withdrawal||10):Number(settings.buffer_return||10);const amount=type==='return'?Math.min(Math.max(0,Number(r.savings_balance||0)),Math.max(0,Number(r.loan?.remaining_balance||0))):Math.round(principal(r)*(pct/100)*100)/100;setEdits(p=>({...p,[r.id]:{...(p[r.id]||blank()),withdrawalType:type,withdrawal:amount>0?String(amount):''}}))};
