...
export default function CoHistoryScreen() {
  ...
  const load = async () => {
    const res = await loadActivities(60);
    setItems(res.data || []);
  ...
  if (loading) {
    return (
      <View style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }]}>
        ...
        <Text style={[styles.loadingText, { color: colors.textSecondary }]}>Loading your history…</Text>
      </View>
    );
  }
      ...
      <FlatList
          ...
          <>
            <View style={[styles.hero, { backgroundColor: colors.card, borderColor: colors.border }]}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={[styles.heroSubtitle, { color: colors.textSecondary }]}>Track your latest field transactions</Text>
              </View>
              <View style={styles.heroStat}>
                <Text style={[styles.heroCount, { color: colors.text }]}>{filteredItems.length}</Text>
                <Text style={[styles.heroLabel, { color: colors.textSecondary }]}>records</Text>
              </View>
            </View>
            ...
            <View style={styles.filterHeader}>
              <Text style={[styles.sectionTitle, { color: colors.text }]}>Filter activity</Text>
              <Text style={[styles.total, { color: colors.textSecondary }]}>{money(totalAmount)}</Text>
            </View>
            <FlatList
              horizontal
                ...
                return (
                  ...
                  >
                    <Text style={{ color: active ? '#fff' : colors.textSecondary, fontSize: 12, fontWeight: '700' }}>
                      {item === 'All' ? 'All' : typeMeta[item]?.label || item}
                    </Text>
                  </Pressable>
                );
              }}
        ...
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={[styles.emptyText, { color: colors.textSecondary }]}>Try another filter or pull down to refresh.</Text>
          </View>
        }
        renderItem={({ item }) => {
          ...
          return (
            <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <View style={[styles.activityIcon, { backgroundColor: meta.color + '16' }]}>
...
const styles = StyleSheet.create({
  ...
  hero: { flexDirection: 'row', alignItems: 'center', borderRadius: RADIUS.lg, borderWidth: 1, padding: 16, marginBottom: 12 },
  heroIcon: { width: 48, height: 48, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginRight: 12 },
  heroTitle: { fontSize: 18, fontWeight: '800' },
  heroSubtitle: { fontSize: 11, marginTop: 3 },
  heroStat: { alignItems: 'flex-end', marginLeft: 8 },
  heroCount: { fontSize: 20, fontWeight: '900' },
  heroLabel: { fontSize: 10 },
  info: { flexDirection: 'row', alignItems: 'flex-start', padding: 12, borderRadius: RADIUS.md, marginBottom: 12 },
  infoText: { flex: 1, fontSize: 12, lineHeight: 18, marginLeft: 8 },
  filterHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginTop: 4 },
  sectionTitle: { fontSize: 14, fontWeight: '800' },
  total: { fontSize: 11, fontWeight: '700' },
  filters: { gap: 8, paddingVertical: 10, paddingBottom: 14 },
  ...
  client: { flex: 1, fontSize: 14, fontWeight: '750' as any },
