...
export async function cacheClients(clients: any[]): Promise<void> {
  ...
  await database.withTransactionAsync(async () => {
    for (const c of clients) {
      await database.runAsync(
        `INSERT INTO clients_cache (id, name, phone, email, status, branch_id, officer_username, raw_json, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(id) DO UPDATE SET
           name=excluded.name, phone=excluded.phone, email=excluded.email,
           status=excluded.status, branch_id=excluded.branch_id,
           officer_username=excluded.officer_username, raw_json=excluded.raw_json,
        ...
        [
          ...
          c.email ?? null,
          c.status ?? null,
          c.branch_id != null ? String(c.branch_id) : null,
          c.officer_username ?? null,
          JSON.stringify(c),
          now,
        ]
      );
...
export async function searchClientsLocal(q: string, limit = 30): Promise<any[]> {
  ...
  const rows = await database.getAllAsync<{ raw_json: string }>(
    `SELECT raw_json FROM clients_cache
...
export async function cacheActivities(items: any[]): Promise<void> {
  ...
  await database.withTransactionAsync(async () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      await database.runAsync(
        `INSERT OR REPLACE INTO activities_cache (id, type, client_name, amount, date, officer, raw_json, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          a.type ?? null,
          a.client_name ?? null,
          Number(a.amount ?? 0),
          a.date != null ? String(a.date) : null,
          a.officer ?? null,
          JSON.stringify(a),
          now,
        ]
...
export async function getActivitiesLocal(limit = 40): Promise<any[]> {
  const database = await getDb();
  const rows = await database.getAllAsync<{ raw_json: string }>(
    `SELECT raw_json FROM activities_cache ORDER BY date DESC LIMIT ?`,
    [limit]
  );
  return rows.map((r) => {
    try {
...
export async function enqueueOp(opType: string, payload: object): Promise<void> {
  ...
  await database.runAsync(
    ...
    [opType, JSON.stringify(payload), Date.now()]
  );
}

export async function listPendingOps(): Promise<
  { id: number; op_type: string; payload: string; tries: number }[]
> {
  const database = await getDb();
  return database.getAllAsync(
    `SELECT id, op_type, payload, tries FROM pending_ops ORDER BY id ASC LIMIT 50`
  );
}
...
export async function bumpPendingOp(id: number, error: string): Promise<void> {
  const database = await getDb();
  await database.runAsync(
    `UPDATE pending_ops SET tries = tries + 1, last_error = ? WHERE id = ?`,
