...
import type { LoginResponse, User, Client, Portfolio, Saving, Loan, Transaction, ApiResponse } from '../types';
...
class ApiClient {
  ...
  constructor() {
    this.client = axios.create({ baseURL: API_BASE_URL, timeout: 20000, headers: { 'Content-Type': 'application/json', Accept: 'application/json' } });
  ...
  async getPortfolio(clientId: string): Promise<Portfolio> { const { data } = await this.client.get<{ success: boolean; data: Portfolio }>(`/clients/${clientId}/portfolio`); if (!data.success || !data.data) throw new ApiError('Failed to load client portfolio.'); return data.data; }
  async getSavings(clientId: string): Promise<Saving[]> { const { data } = await this.client.get<ApiResponse<Saving[]>>(`/clients/${clientId}/savings`); return data.data || []; }
  async getLoans(clientId: string): Promise<Loan[]> { const { data } = await this.client.get<ApiResponse<Loan[]>>(`/clients/${clientId}/loans`); return data.data || []; }
  async getTransactions(clientId: string): Promise<Transaction[]> { const { data } = await this.client.get<ApiResponse<Transaction[]>>(`/clients/${clientId}/transactions`); return data.data || []; }
