...
export class ApiError extends Error {
  ...
  constructor(message: string, options: { status?: number; code?: string; offline?: boolean } = {}) {
    super(message); this.name = 'ApiError'; this.status = options.status; this.code = options.code; this.offline = options.offline ?? false;
  }
}
...
function normalizeError(error: unknown): ApiError {
  ...
  if (!e?.response && (e?.code === 'ERR_NETWORK' || e?.code === 'ECONNABORTED' || e?.message?.toLowerCase().includes('network'))) return new ApiError('Unable to connect to CUPAD. Check your internet connection.', { code: e.code, offline: true });
...
class ApiClient {
  ...
  constructor() {
    this.client = axios.create({ baseURL: API_BASE_URL, timeout: API_TIMEOUT_MS, headers: { 'Content-Type': 'application/json', Accept: 'application/json' } });
    this.client.interceptors.request.use(async (config) => { const token = await storageGet(TOKEN_KEY); if (token) config.headers.Authorization = `Bearer ${token}`; return config; });
    this.client.interceptors.response.use((response) => response, async (error: AxiosError) => { if (error.response?.status === 401) await this.clearToken(); return Promise.reject(normalizeError(error)); });
  }
  ...
  async updateProfile(payload: { full_name?: string; email?: string; current_password?: string; new_password?: string; profile_pic?: string }): Promise<User> {
    // POST is used for profile updates because it works consistently through Apache/proxy layers; backend accepts POST and PUT.
    const { data } = await this.client.post<{ success: boolean; data: User; error?: string; message?: string }>('/profile', payload);
    if (!data.success || !data.data) throw new ApiError(data.error || data.message || 'Profile update failed.');
    return data.data;
  }
